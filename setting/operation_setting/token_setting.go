...
func (s TokenSetting) Validate() error {
	if s.MaxUserTokens < 0 || int64(s.MaxUserTokens) > math.MaxInt32 {
		return fmt.Errorf("maximum user tokens must be in the range [0, %d]", int64(math.MaxInt32))
	}
	return nil
}
...
func GetTokenSetting() *TokenSetting {
	return config.Snapshot[TokenSetting]("token_setting")
}
