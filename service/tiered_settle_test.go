...
func TestPrepareTieredBillingForSelectedGroupTopUpArrearsAllowsNegativeBalance(t *testing.T) {
	...
	relayInfo := &relaycommon.RelayInfo{
		UserId:                userID,
		IsPlayground:          true,
		RequestId:             "tiered-topup-insufficient-wallet",
		OriginModelName:       "gpt-test",
		FinalPreConsumedQuota: 50_000,
		TieredBillingSnapshot: &billingexpr.BillingSnapshot{
			BillingMode:               "tiered_expr",
	...
	_, err := model.CreateBillingReservation(model.BillingReservationRequest{
		RequestID:      relayInfo.RequestId,
		UserID:         userID,
		FundingSource:  BillingSourceWallet,
		ModelName:      relayInfo.OriginModelName,
		IsPlayground:   true,
		RequestedQuota: 50_000,
		InitialQuota:   50_000,
	})
	require.NoError(t, err)
	...
	require.NotNil(t, PrepareTieredBillingForSelectedGroup(nil, relayInfo))

	// Durable preflight reservation rejects an unfunded top-up instead of
	// sending a request that would put the wallet in arrears.
...
func TestBillingSessionReserveWalletTopUpDecrementsBalance(t *testing.T) {
	...
	relayInfo := &relaycommon.RelayInfo{
		UserId:          userID,
		IsPlayground:    true,
		RequestId:       "tiered-reserve-wallet-topup",
		OriginModelName: "gpt-test",
	}
	_, err := model.CreateBillingReservation(model.BillingReservationRequest{
		RequestID:      relayInfo.RequestId,
