...
func TestOpenAIResponsesToClaudePreservesStrictToolsAndStructuredOutput(t *testing.T) {
	request, err := OpenAIResponsesRequestToClaudeMessages(nil, &dto.OpenAIResponsesRequest{
...
func TestOpenAIResponsesToClaudeRejectsUnsupportedMedia(t *testing.T) {
	_, err := OpenAIResponsesRequestToClaudeMessages(nil, &dto.OpenAIResponsesRequest{
		Model: "claude-sonnet-4",
		Input: mustRawMessage(t, []map[string]any{{
			"role": "user",
