...
const GROUP_FALLBACK_TARGET_RATIO_MODES = new Set([
  'origin_special',
  'target_special',
  'normal_only',
...
function validateGroupFallback(value) {
  ...
  try {
    ...
    for (const [sourceGroup, rule] of Object.entries(parsed)) {
      ...
      if (!rule.fallback.every((item) => typeof item === 'string'))
        return false;
      if (
        rule.pricing_mode &&
      ...
      if (
        'origin_pricing_use_special_ratio' in rule &&
        typeof rule.origin_pricing_use_special_ratio !== 'boolean'
      ) {
        return false;
      }
      if (
        'target_pricing_ratio_mode' in rule &&
        (typeof rule.target_pricing_ratio_mode !== 'string' ||
          !GROUP_FALLBACK_TARGET_RATIO_MODES.has(
  ...
  const renderVisualMode = () => (
    <Form key='form-visual' values={inputs} style={{ marginBottom: 15 }}>
      ...
      <Form.Section text={t('分组回退')}>
        ...
        >
          {t(
            '分组回退规则映射为 JSON：源分组到回退策略，fallback 为回退分组数组，pricing_mode 可选 origin 或 target，并可配置特殊倍率处理方式',
          )}
        </Text>
        <GroupFallbackRules
          key={`gfr_${dv}`}
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              label={t('分组回退')}
              placeholder={t(
                '为一个 JSON 文本，键为源分组，值为包含 fallback、pricing_mode 和特殊倍率处理字段的对象',
              )}
              extraText={t(
                '回退配置格式示例：{"vip": {"fallback": ["default","enterprise"], "pricing_mode": "target", "target_pricing_ratio_mode": "target_special"}}',
              )}
              field={'GroupFallback'}
              autosize={{ minRows: 6, maxRows: 12 }}
              trigger='blur'
