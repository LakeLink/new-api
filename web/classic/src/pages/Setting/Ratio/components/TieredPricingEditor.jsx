...
function tierConditionsMatch(tier, variables) {
...
function evaluateTierCost(tier, variables) {
  let cost =
    variables.p * Number(tier.input_unit_cost || 0) +
    variables.c * Number(tier.output_unit_cost || 0);

  for (const variable of CACHE_VAR_MAP) {
    cost += variables[variable.exprVar] * Number(tier[variable.field] || 0);
  }

  return cost;
}

function evaluateVisualConfig(config, variables) {
  if (config.tiers.length === 1) {
    const tier = config.tiers[0];
    if (
      (tier.conditions || []).length > 0 &&
      !tierConditionsMatch(tier, variables)
    ) {
      return { cost: 0, matchedTier: '' };
    }
    return {
      cost: evaluateTierCost(tier, variables),
  ...
  for (let index = 0; index < config.tiers.length; index += 1) {
    ...
    const isFallback =
      index === config.tiers.length - 1 || (tier.conditions || []).length === 0;

    if (isFallback || tierConditionsMatch(tier, variables)) {
      return {
        cost: evaluateTierCost(tier, variables),
...
function evalExprLocally(exprStr, p, c, extraTokenValues) {
  ...
  if (!visualConfig) {
    return { cost: 0, matchedTier: '', error: 'unsupported_expression' };
  }

  const variables = {
    p: Number.isFinite(p) ? Math.max(0, p) : 0,
    c: Number.isFinite(c) ? Math.max(0, c) : 0,
  };
  for (const field of EXTRA_ESTIMATOR_FIELDS) {
    const value = extraTokenValues[field.stateKey];
    variables[field.var] = Number.isFinite(value) ? Math.max(0, value) : 0;
  }
  variables.len = variables.p + variables.cr + variables.cc + variables.cc1h;

  const result = evaluateVisualConfig(visualConfig, variables);
  if (!Number.isFinite(result.cost) || result.cost < 0) {
    return { cost: 0, matchedTier: '', error: 'invalid_result' };
  }
