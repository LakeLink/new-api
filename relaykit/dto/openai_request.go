...
// MaxChatCompletionsN caps the number of generated choices. This bounds every
// request field that multiplies completion tokens in quota calculation.
...
func (r *GeneralOpenAIRequest) GetTokenCountMeta() *types.TokenCountMeta {
	...
	if strings.HasPrefix(r.Model, "imagen-") {
		...
		if len(r.ExtraBody) > 0 {
			...
			if err := kitutil.Unmarshal(r.ExtraBody, &extra); err == nil {
				var extraN *int
				if rawN, exists := extra["n"]; exists && kitutil.Unmarshal(rawN, &extraN) == nil &&
