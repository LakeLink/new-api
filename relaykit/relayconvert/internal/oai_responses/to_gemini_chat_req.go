...
func OpenAIResponsesRequestToGeminiChat(c context.Context, req *dto.OpenAIResponsesRequest, info convmeta.Meta) (*dto.GeminiChatRequest, error) {
	...
	geminiRequest.GenerationConfig.MaxOutputTokens = req.MaxOutputTokens
	if req.ServiceTier != "" {
		convertedTier, ok := sharedgemini.OpenAIServiceTierToGemini(req.ServiceTier)
		if !ok {
			return nil, fmt.Errorf("service_tier %q cannot be converted to Gemini", req.ServiceTier)
		}
		if convertedTier != "" {
			geminiRequest.ServiceTier = kitutil.GetPointer(convertedTier)
		}
	}
	if RawJSONPresent(req.Store) {
		var store bool
		if err := kitutil.Unmarshal(req.Store, &store); err != nil {
	...
	for _, item := range inputItems {
		...
		case ResponsesInputTypeCustomToolCall, ResponsesInputTypeCustomToolOutput:
			return nil, fmt.Errorf("Responses input item type %q cannot be converted to Gemini", itemType)
		default:
