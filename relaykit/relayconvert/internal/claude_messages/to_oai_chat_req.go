...
func ClaudeMessagesRequestToOpenAIChat(claudeRequest dto.ClaudeRequest, info convmeta.Meta) (*dto.GeneralOpenAIRequest, error) {
	...
	if claudeRequest.ToolChoice != nil {
		toolChoice, err := kitutil.Any2Type[dto.ClaudeToolChoice](claudeRequest.ToolChoice)
		if err != nil {
	...
	if claudeRequest.Tools != nil {
		encodedTools, err := kitutil.Marshal(claudeRequest.Tools)
		if err != nil {
			return nil, fmt.Errorf("invalid Claude tools: %w", err)
		}
		var rawTools []map[string]any
		if err := kitutil.Unmarshal(encodedTools, &rawTools); err != nil {
			return nil, fmt.Errorf("invalid Claude tools: %w", err)
		}
		for index, rawTool := range rawTools {
			toolType := kitutil.Interface2String(rawTool["type"])
			if toolType == "web_search_20250305" && kitutil.Interface2String(rawTool["name"]) == "web_search" {
				if openAIRequest.WebSearchOptions == nil {
					openAIRequest.WebSearchOptions = &dto.WebSearchOptions{}
				}
				maxUses, _ := strconv.Atoi(kitutil.Interface2String(rawTool["max_uses"]))
				switch maxUses {
				case webSearchMaxUsesLow:
					openAIRequest.WebSearchOptions.SearchContextSize = "low"
				case webSearchMaxUsesHigh:
					openAIRequest.WebSearchOptions.SearchContextSize = "high"
				case 0, webSearchMaxUsesMedium:
				...
				if location, ok := rawTool["user_location"].(map[string]any); ok {
					locationRaw, err := kitutil.Marshal(map[string]any{"approximate": location})
					if err != nil {
			...
			rawToolJSON, err := kitutil.Marshal(rawTool)
			if err != nil {
				return nil, fmt.Errorf("invalid Claude tools[%d]: %w", index, err)
			}
			if err := kitutil.Unmarshal(rawToolJSON, &claudeTool); err != nil {
				return nil, fmt.Errorf("invalid Claude tools[%d]: %w", index, err)
			}
			if claudeTool.Name == "" {
...
func applyClaudeStructuredOutputToOpenAI(request *dto.GeneralOpenAIRequest, claudeRequest dto.ClaudeRequest) error {
	...
	for _, raw := range []json.RawMessage{claudeRequest.OutputConfig, claudeRequest.OutputFormat} {
		if len(raw) == 0 {
			continue
		}
	...
	if kitutil.Interface2String(format["type"]) != "json_schema" {
		return fmt.Errorf("Claude output format %q cannot be converted to OpenAI Chat", kitutil.Interface2String(format["type"]))
	}
	...
	encoded, err := kitutil.Marshal(jsonSchema)
	if err != nil {
...
func claudeImageToOpenAIMedia(mediaMsg dto.ClaudeMediaMessage) (dto.MediaContent, error) {
	...
	case "base64":
		...
		imageURL = fmt.Sprintf("data:%s;base64,%s", mediaMsg.Source.MediaType, kitutil.Interface2String(mediaMsg.Source.Data))
	default:
...
func claudeDocumentToOpenAIMedia(mediaMsg dto.ClaudeMediaMessage) (dto.MediaContent, error) {
	...
	return dto.MediaContent{
		...
		File: &dto.MessageFile{
			...
			FileData: kitutil.Interface2String(mediaMsg.Source.Data),
		},
	}, nil
}
