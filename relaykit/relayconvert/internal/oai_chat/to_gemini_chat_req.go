...
func OpenAIChatRequestToGeminiGenerateContent(c context.Context, textRequest dto.GeneralOpenAIRequest, info convmeta.Meta) (*dto.GeminiChatRequest, error) {
	...
	if textRequest.TopLogProbs != nil {
		if textRequest.LogProbs == nil || !*textRequest.LogProbs {
			return nil, errors.New("top_logprobs requires logprobs must be true")
		}
		logprobs := int32(*textRequest.TopLogProbs)
		geminiRequest.GenerationConfig.Logprobs = &logprobs
	}
	if len(textRequest.ServiceTier) > 0 {
		var serviceTier string
		if err := kitutil.Unmarshal(textRequest.ServiceTier, &serviceTier); err != nil {
			return nil, fmt.Errorf("invalid service_tier: %w", err)
		}
		convertedTier, ok := sharedgemini.OpenAIServiceTierToGemini(serviceTier)
		if !ok {
			return nil, fmt.Errorf("service_tier %q cannot be converted to Gemini", serviceTier)
		}
		if convertedTier != "" {
			geminiRequest.ServiceTier = kitutil.GetPointer(convertedTier)
		}
	}
	if len(textRequest.Store) > 0 {
		var store bool
		if err := kitutil.Unmarshal(textRequest.Store, &store); err != nil {
			return nil, fmt.Errorf("invalid store: %w", err)
		}
		geminiRequest.Store = &store
	}
			...
			if !strings.HasSuffix(upstreamModelName, "-nothinking") {
				...
				if thinkingConfig, ok := googleBody["thinking_config"].(map[string]interface{}); ok {
					...
					if thinkingBudget, exists := thinkingConfig["thinking_budget"]; exists {
						...
						case float64:
							if math.IsInf(v, 0) || math.IsNaN(v) || math.Trunc(v) != v || v < -1 || v > float64(dto.MaxGeminiThinkingBudget) {
								return nil, fmt.Errorf("extra_body.google.thinking_config.thinking_budget must be an integer between -1 and %d", dto.MaxGeminiThinkingBudget)
							}
							budgetInt := int(v)
							tempThinkingConfig.ThinkingBudget = kitutil.GetPointer(budgetInt)
							tempThinkingConfig.IncludeThoughts = kitutil.GetPointer(budgetInt > 0)
	...
	for _, message := range textRequest.Messages {
		...
		for _, part := range openaiContent {
			if part.Type == dto.ContentTypeText {
				...
				if err != nil {
					return nil, fmt.Errorf("get file data from '%s' failed: %w", kitutil.MaskSensitiveInfo(source.GetIdentifier()), err)
				}

				if _, ok := sharedgemini.SupportedMimeTypes[strings.ToLower(mimeType)]; !ok {
					return nil, fmt.Errorf("mime type is not supported by Gemini: '%s', url: '%s', supported types are: %v", mimeType, kitutil.MaskSensitiveInfo(source.GetIdentifier()), sharedgemini.SupportedMimeTypesList())
				}
