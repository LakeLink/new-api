...
func ChatCompletionsRequestToResponsesRequest(req *dto.GeneralOpenAIRequest) (*dto.OpenAIResponsesRequest, error) {
	...
	if len(req.Verbosity) > 0 {
		textOptions := make(map[string]any)
		if len(textRaw) > 0 {
			if err := kitutil.Unmarshal(textRaw, &textOptions); err != nil {
				return nil, fmt.Errorf("invalid responses text options: %w", err)
			}
		}
		var verbosity any
		if err := kitutil.Unmarshal(req.Verbosity, &verbosity); err != nil {
			return nil, fmt.Errorf("invalid verbosity: %w", err)
		}
		textOptions["verbosity"] = verbosity
		encodedText, err := kitutil.Marshal(textOptions)
		if err != nil {
			return nil, fmt.Errorf("marshal responses text options: %w", err)
		}
		textRaw = encodedText
	}
	...
	if req.PromptCacheKey != "" {
		encodedPromptCacheKey, err := kitutil.Marshal(req.PromptCacheKey)
		if err != nil {
			return nil, fmt.Errorf("marshal prompt_cache_key: %w", err)
		}
		promptCacheKey = encodedPromptCacheKey
	}

	var reasoning *dto.Reasoning
	if len(req.Reasoning) > 0 {
		parsedReasoning := dto.Reasoning{}
		if err := kitutil.Unmarshal(req.Reasoning, &parsedReasoning); err != nil {
			return nil, fmt.Errorf("invalid reasoning: %w", err)
		}
		reasoning = &parsedReasoning
	}
	if req.ReasoningEffort != "" {
		if reasoning == nil {
			reasoning = &dto.Reasoning{Summary: "detailed"}
		}
		if reasoning.Effort == "" {
	...
	out := &dto.OpenAIResponsesRequest{
		...
		ServiceTier:       kitutil.JsonRawMessageToString(req.ServiceTier),
		Store:             req.Store,
	...
	if req.MaxTokens != nil || req.MaxCompletionTokens != nil {
		maxOutputTokens := openAIChatMaxTokens(*req)
