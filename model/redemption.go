...
func Redeem(key string, userId int) (quota int, err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := redemption.validateQuota(); err != nil {
			return err
		}
		var user User
		...
		newQuota, err := checkedQuotaBalanceDelta(user.Quota, redemption.Quota)
		if err != nil {
			return err
		}
	...
	if err := InvalidateUserCache(userId); err != nil {
		common.SysLog("failed to invalidate user cache after redemption: " + err.Error())
	}
...
func (redemption *Redemption) validateQuota() error {
	if redemption.Quota <= 0 || redemption.Quota > common.MaxQuota {
		return errors.New("redemption quota must be positive and within the supported range")
	}
	return nil
}
